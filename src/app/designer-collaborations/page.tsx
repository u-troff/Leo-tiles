import type { Metadata } from "next";
import Image from "next/image";
import SectionLogo from "@/components/ui/SectionLogo";
import Button from "@/components/ui/Button";
import { siteConfig } from "@/data/site-config";
import { collaborations } from "@/data/collaborations";

export const metadata: Metadata = {
  title: "Designer Collaborations",
  description: `Celebrating the interior designers and architects ${siteConfig.businessName} has collaborated with since ${siteConfig.establishedYear}, from Franschhoek studios to hospitality projects.`,
};

export default function DesignerCollaborationsPage() {
  return (
    <>
      <section className="py-16 lg:py-24 bg-clay-900">
        <div className="max-w-7xl mx-auto px-4">
          <SectionLogo label="Designer Collaborations" />
          <h1 className="text-3xl lg:text-5xl font-bold text-stone-50 mb-6 max-w-2xl leading-tight">
            Celebrating the Designers We Build With
          </h1>
          <p className="text-stone-200 text-lg leading-relaxed max-w-2xl mb-4">
            Over 50 years, {siteConfig.businessName} has worked alongside interior designers and
            architects across South Africa, some of them among the most respected names in the
            industry. Since we can&rsquo;t always ask for a formal testimonial, this page showcases
            the projects themselves.
          </p>
          <div className="flex gap-4 mt-8">
            <Button href="/get-a-quote" variant="primary" size="lg">
              Get a Quote
            </Button>
            <Button href={`tel:${siteConfig.phoneRaw}`} variant="secondary" size="lg">
              Call Us
            </Button>
          </div>
        </div>
      </section>

      <section className="py-16 lg:py-24 bg-stone-50">
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {collaborations.map((collab) => (
              <div key={collab.slug} className={`overflow-hidden ${collab.comingSoon ? "opacity-80" : ""}`}>
                <div className="relative h-[220px] mb-4">
                  <Image
                    src={collab.image}
                    alt={collab.partnerName}
                    fill
                    className={`object-cover ${collab.comingSoon ? "grayscale-[35%]" : ""}`}
                    sizes="(max-width: 1024px) 100vw, 33vw"
                  />
                  {collab.comingSoon && (
                    <span className="absolute top-3 left-3 bg-clay-900 text-stone-50 text-[11px] font-bold uppercase tracking-wide px-2.5 py-1">
                      Coming Soon
                    </span>
                  )}
                </div>
                <div className="text-clay-900 font-bold text-lg mb-1">{collab.partnerName}</div>
                <div className="text-rust-500 text-sm font-semibold mb-2">
                  {collab.partnerType} · {collab.location}
                  {collab.projectCount && ` · ${collab.projectCount} projects`}
                </div>
                {collab.projectHighlight && (
                  <div className="text-stone-500 text-[13px] font-semibold uppercase tracking-wide mb-2">
                    {collab.projectHighlight}
                  </div>
                )}
                <p className="text-stone-700 text-[15px] leading-relaxed">{collab.description}</p>
              </div>
            ))}
          </div>
        </div>
      </section>

      <section className="py-16 bg-stone-100 text-center px-4">
        <h2 className="text-2xl lg:text-3xl font-bold text-clay-900 mb-3.5">
          Working on a project with a designer or architect?
        </h2>
        <p className="text-stone-700 mb-7 max-w-xl mx-auto">
          We supply samples, technical data sheets, and trade accounts for design professionals.
        </p>
        <Button href="/get-a-quote" variant="primary" size="lg">
          Start a Conversation
        </Button>
      </section>
    </>
  );
}
